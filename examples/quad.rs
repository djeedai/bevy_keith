...
fn draw_button(
    ...
    let (brush, border_brush) = if rect.contains(cursor_pos) {
        let border_brush = ctx.solid_brush(Color::srgb(0.6, 0.6, 1.0));
        (ctx.solid_brush(Color::srgb(0.7, 0.7, 0.7)), border_brush)
    } else {
        let border_brush = ctx.solid_brush(Color::srgb(0.8, 0.8, 0.9));
        (ctx.solid_brush(Color::srgb(0.6, 0.6, 0.6)), border_brush)
    };
